...
int evt_chamb_las_fpp(){
	...
	chamb_ray(chamb);
	dr_set_r(&dr_data,&dr_data_index[1],"LF_RAYID",0,ray->rayid);
	dr_set_r(&dr_data,&dr_data_index[2],"LF_CHI2",0,chamb->chi2);
	dr_set_r(&dr_data,&dr_data_index[3],"LF_NCOMB",0,chamb->ncomb);
	if(ray->rayid==RAYID_NOERR){
		/* rotation of axes as z=central-ray */
		ray_rotate_z(ray,dr_get_r(&dr_data,&dr_data_index[4],"LF_TILT",2));
		ray_rotate_y(ray,dr_get_r(&dr_data,&dr_data_index[5],"LF_TILT",1));
		ray_rotate_x(ray,dr_get_r(&dr_data,&dr_data_index[6],"LF_TILT",0));
		/* output of ray information */
		dr_set_r(&dr_data,&dr_data_index[7],"LF_RAY_X",0, ray->org.x);
		dr_set_r(&dr_data,&dr_data_index[8],"LF_RAY_Y",0, ray->org.y);
		dr_set_r(&dr_data,&dr_data_index[9],"LF_TH",0,atan(ray->dir.x));
		dr_set_r(&dr_data,&dr_data_index[10],"LF_PH",0,atan(ray->dir.y));
	...
	if(lray->rayid==RAYID_NOERR){
		if(ray_vertex(ray,lray,&p,&dist)==0){
			dr_set_r(&dr_data,&dr_data_index[11],"LF_VERTEX_X",0, p.x);
			dr_set_r(&dr_data,&dr_data_index[12],"LF_VERTEX_Y",0, p.y);
			dr_set_r(&dr_data,&dr_data_index[13],"LF_VERTEX_Z",0, p.z);
		}
		dr_set_r(&dr_data,&dr_data_index[14],"LF_DIST",0, dist);  /* if error, error code is saved in 'dist' */
		if(ray_scat_ang(lray,ray,&ath,&aph,&hor,&ver)==0){
			/* scattering angle (all in radian) */
			dr_set_r(&dr_data,&dr_data_index[15],"LF_SCAT_TH",0, ath);  /* scattering angld (polar angle) */
			dr_set_r(&dr_data,&dr_data_index[16],"LF_SCAT_PH",0, aph);  /* scattering angld (azimuthal angle) */
			dr_set_r(&dr_data,&dr_data_index[17],"LF_SCAT_DX",0, hor);  /* scattering angld (horizontal) */
			dr_set_r(&dr_data,&dr_data_index[18],"LF_SCAT_DY",0, ver);  /* scattering angld (vertical) */
		}
	}
	return(0);
}
...
int evt_start_las_fpp(){
  ...
  chamb_get_config(chamb);
	for(ipl=0; ipl<chamb->npl; ipl++){
		...
		plane->fit = dr_is_true(dr_get_r(&dr_data,&dr_data_index[19],"LF_FIT_PLANE",ipl));
	}
	if(dr_exists(d=dr_get_r(&dr_data,&dr_data_index[20],"LF_MAX_NCOMB",0))) chamb->max_ncomb = (int)d;
	chamb->allow_wireskip = dr_is_true(dr_get_r(&dr_data,&dr_data_index[21],"LF_ALLOW_WIRESKIP",0));
	d = dr_get_r(&dr_data,&dr_data_index[22],"LF_N_VPLANE",0);
