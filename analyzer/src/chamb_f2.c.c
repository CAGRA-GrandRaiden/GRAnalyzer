...
int evt_chamb_f2(){
	...
	chamb_ray(chamb);	
	dr_set_r(&dr_data,&dr_data_index[0],"F2_RAYID",0,ray->rayid);
	if(ray->rayid==RAYID_NOERR){
    /* rotation of axes as z=central-ray */
		ray_rotate_z(ray,dr_get_r(&dr_data,&dr_data_index[1],"F2_MWDC_TILT",2));
		ray_rotate_y(ray,dr_get_r(&dr_data,&dr_data_index[2],"F2_MWDC_TILT",1));
		ray_rotate_x(ray,dr_get_r(&dr_data,&dr_data_index[3],"F2_MWDC_TILT",0));
    /* output of ray information */
		dr_set_r(&dr_data,&dr_data_index[4],"F2_RAY_X",0, ray->org.x);
		dr_set_r(&dr_data,&dr_data_index[5],"F2_RAY_Y",0, ray->org.y);
		dr_set_r(&dr_data,&dr_data_index[6],"F2_TH",0,atan(ray->dir.x));
		dr_set_r(&dr_data,&dr_data_index[7],"F2_PH",0,atan(ray->dir.y));
...
int evt_start_f2(){
  ...
  chamb_get_config(chamb);
	for(ipl=0; ipl<chamb->npl; ipl++){
		...
		plane->fit = dr_is_true(dr_get_r(&dr_data,&dr_data_index[8],"F2_FIT_PLANE",ipl));
	}
	if(dr_exists(d=dr_get_r(&dr_data,&dr_data_index[9],"F2_MAX_NCOMB",0))) chamb->max_ncomb = (int)d;
	chamb->allow_wireskip = dr_is_true(dr_get_r(&dr_data,&dr_data_index[10],"F2_ALLOW_WIRESKIP",0));
	d = dr_get_r(&dr_data,&dr_data_index[11],"F2_N_VPLANE",0);
