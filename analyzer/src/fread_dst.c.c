...
static int dst_open_file()
{
	...
	f  = dr_get_sval_r(&dr_data,&dr_data_index[0],"DST_FILE",0);
...
static int dst_write_header(char const *comment)
{
	...
	/* variables to write */
	name = dr_get_sval_r(&dr_data,&dr_data_index[1],"DST_VAR",0);
	ndstvar = 0;
	if(name){
		d = p = name;
...
int dst_write_data()
{
	...
	if(!fd_write)
		return(0);
	if(dr_is_false(dr_get_r(&dr_data,&dr_data_index[2],"DST_CONDITION",0))) 
		 return(0);
	iend = IND_END;
...
#define SWAP 0
	...
	dr_clear();
	neval++;
	dr_set_r(&dr_data,&dr_data_index[3],"EVENT_DST",0, 1);
	pos = *bpos;
	for(iv=0;;){
		/* read index */
		if(pos>=size){
	...
	*bpos = pos;
	if(dr_exists(d=dr_get_r(&dr_data,&dr_data_index[4],"RUN_N",0)))
		nrun = (int)d;
	if(dr_exists(d=dr_get_r(&dr_data,&dr_data_index[5],"BLOCK_N",0)))
		blk = (int)d;
	else
		blk = 0;
	if(ana<0 || nblk!=blk){
		...
		fprintf(stderr, "%d %d %f %f\n", blk, ana,
						dr_get_r(&dr_data,&dr_data_index[6],"ANA_BLK_START",0), dr_get_r(&dr_data,&dr_data_index[7],"ANA_BLK_END",0));
#endif
	}
	if(ana){
		if(pflag){
		...
		if(dr_is_true(dr_get_r(&dr_data,&dr_data_index[8],"ANALYZE_SHOW_SCALER",0)))
